...
import com.fisei.MiPrimeraApp.services.ImageService;
...
public class ShopItemArrayAdapter extends ArrayAdapter<ShopCartItem> {
    ...
    private void DeleteFromCart(int id){
        for (int x = 0; x < MiAppGlobal.SHOPPING_CART.size(); x++){
            if (MiAppGlobal.SHOPPING_CART.get(x).Id == id){
                MiAppGlobal.SHOPPING_CART.remove(x);
                break;
            }
        }
        UpdateArrayAdapter();
